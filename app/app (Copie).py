...
def home():
    ...
    recent_data = db.session.query(
        ...
        Rating.commentaire != '',
        Rating.is_public == True,   # ← ajoute cette ligne
        Artwork.image_url.isnot(None),
        Artwork.image_url != ''
...
def favorites_page():
    ...
    # Pour le tri date_added, on fetch tout puis on trie en Python
    if sort == 'date_added':
...
def api_favorites_works():
    """Endpoint de pagination pour la page favoris"""
    if 'user_id' not in session:
        return jsonify({'error': 'Non connecté'}), 401
...
def api_favorites_filter_options():
    """
    Retourne les options de filtre disponibles DANS les favoris de l'utilisateur.
    Utilisé par la page /favoris pour peupler les dropdowns.
    """
    if 'user_id' not in session:
        return jsonify({'error': 'Non connecté'}), 401
    ...
    try:
        ...
        if not fav_ids:
            return jsonify({'artists': [], 'countries': [], 'cities': [], 'museums': [], 'types': []})

        filter_type = request.args.get('filter', 'all')  # artists | countries | cities | museums | types
        ...
        if filter_type in ('all', 'artists'):
            ...
            result['artists'] = [{'id': a.name, 'name': a.name, 'count': a.count, 'selected': a.name in selected_artists} for a in artists]

        # PAYS
        if filter_type in ('all', 'countries'):
            ...
            ).join(Artwork, Artwork.id == ArtworkCollection.artwork_id
            ).filter(
                Artwork.id.in_(fav_ids),
                country_field.isnot(None),
                country_field != ''
            ).group_by(country_field).order_by(func.count(func.distinct(Artwork.id)).desc()).limit(10).all()
            result['countries'] = [{'id': c.name, 'name': c.name, 'count': c.count, 'selected': c.name == selected_country} for c in countries]

        # VILLES
        if filter_type in ('all', 'cities'):
            ...
            cities = db.session.query(
                ...
                city_field != ''
            ).group_by(city_field).order_by(func.count(func.distinct(Artwork.id)).desc()).limit(20).all()
            result['cities'] = [{'id': c.name, 'name': c.name, 'count': c.count, 'selected': c.name in selected_cities} for c in cities]

        # MUSÉES
        if filter_type in ('all', 'museums'):
            ...
            museums = db.session.query(
                ...
                museum_field != ''
            ).group_by(Collection.id, museum_field).order_by(func.count(func.distinct(Artwork.id)).desc()).limit(30).all()
            result['museums'] = [{'id': m.id, 'name': m.name, 'count': m.count, 'selected': m.id in selected_museums} for m in museums]

        # TYPES
        if filter_type in ('all', 'types'):
            type_field = Artwork.instance_of_fr if lang == 'fr' else Artwork.instance_of_en
            selected_types = request.args.getlist('type')
            types = db.session.query(
                ...
                type_field.isnot(None),
                type_field != ''
            ).group_by(type_field).order_by(func.count(Artwork.id).desc()).limit(20).all()
            result['types'] = [{'name': t.name, 'count': t.count, 'selected': t.name in selected_types} for t in types]
